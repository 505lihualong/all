...
import xx.all.common.security.core.properties.SecurityConstants;
import xx.all.common.security.core.properties.SecurityProperties;
import xx.all.common.security.core.properties.SmsCodeProperties;
import xx.all.common.security.core.validate.code.ValidateCode;
import xx.all.common.security.core.validate.code.ValidateCodeException;
import xx.all.common.security.core.validate.code.ValidateCodeGenerator;
import xx.all.common.security.core.validate.code.ValidateCodeRepository;
import xx.all.common.security.core.validate.code.impl.AbstractValidateCodeProcessor;
...
public class SmsCodeProcessor extends AbstractValidateCodeProcessor<ValidateCode> {
	...
	protected void send(ServletWebRequest request, ValidateCode validateCode) throws Exception {
		...
		try {
			...
			logger.error("校验短信数量, e={}", e.getMessage(), e);
	...
	private void checkSendSmsCount(String mobile, String ipAddr) {
		...
		if (sendSmsRateCount != null) {
			logger.error("操作频率过快 ipAddr={}, mobile={}", ipAddr, mobile);
		...
		if (mobileSmsCount != null && mobileSmsCount > sms.getMobileMaxSendCount()) {
			logger.error("Mobile当天短信发送数上限 ipAddr={}, mobile={}", ipAddr, mobile);
		...
		if (ipSmsCount != null && ipSmsCount > sms.getIpMaxSendCount()) {
			logger.error("IP当天短信发送数上限 ipAddr={}, mobile={}", ipAddr, mobile);
		...
		if (totalSmsCount != null && totalSmsCount > sms.getTotalMaxSendCount()) {
			logger.error("当天短信发送数上限 ipAddr={}, mobile={}", ipAddr, mobile);
